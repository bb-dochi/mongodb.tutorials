...
userRouter.put("/:userId", async (req, res) => {
    try {
        ...
        if (!name && !age) return res.status(400).send({ err: "age or name is required." });
        if (name && (typeof name.first !== "string" || typeof name.last !== "string")) return res.status(400).send({ err: "name must be a string" });
